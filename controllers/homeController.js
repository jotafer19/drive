...
exports.homeGet = async (req, res) => {
    ...
    const folders = await folderQuery.getNestedFolders(req.user.id, currentFolderId)
    const folderRoute = await folderQuery.getFolderRoute(req.user.id, currentFolderId)
    ...
    res.render("home", {
        parentFolder: currentFolderId ? await folderQuery.getFolder(req.user.id, currentFolderId) : null,
        folders: folders,
...
exports.addFilePost = async (req, res) => {
    ...
    await fileQuery.addFile(userId, folderId, originalname, size, mimetype, supabaseName, fileURL.signedUrl)

    res.redirect("/uploads/" + (folderId ? folderId : ""))
}

exports.editFilePost = async (req, res) => {
    const userId = req.user.id;
